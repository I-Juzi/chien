...
Chien Admin 是一个基于前后端分离架构的后台权限管理系统，采用 RBAC（基于角色的访问控制）模型，支持动态菜单、JWT 双 Token 认证、在线用户管理、操作日志、数据字典、文件管理、通知公告（SSE 实时推送）等功能。
...
| JWT (jjwt) | 0.12.6 | Token 认证（双 Token：访问令牌 + 刷新令牌） |
...
│       │   └── AutoFillHandler.java     # MyBatis-Plus 自动填充
│       ├── aspect/
...
│       │   ├── FileController.java      # 文件上传/管理
│       │   └── SseController.java       # SSE 推送订阅
│       ├── domain/
│       │   ├── entity/                  # 实体类
│       │   └── vo/                      # 视图对象
│       ├── mapper/                      # MyBatis-Plus Mapper
│       └── service/                     # 业务逻辑层（含 SseService）
│
├── chien-log/                           # 日志模块
│   └── src/main/java/.../
│       ├── aspect/
...
└── chien-ui/                            # 前端模块
    ...
    └── src/
        ...
        ├── components/
        │   └── IconRenderer.vue         # 通用图标组件
        └── views/
            ├── login/                   # 登录页
            ├── dashboard/               # 首页（含系统公告展示）
            ├── system/
            │   ├── user/               # 用户管理
            │   ├── role/               # 角色管理
            │   ├── menu/               # 菜单管理
...
```
chien-common  ←  chien-system  ←  chien-log
                                   ↓
                              chien-admin（启动）
```
...
| 模块 | 职责 | 依赖 |
|------|------|------|
| chien-common | 统一返回结果、错误码、自定义异常、日志注解、限流注解 | 无 |
| chien-system | 用户/角色/菜单/字典/文件管理 + 全局异常处理 + XSS 过滤 + 限流 | chien-common |
| chien-log | 操作日志、登录日志、日志切面 | chien-system |
| chien-admin | 启动类、安全认证、JWT、在线用户、CORS、Swagger | chien-system + chien-log |
...
| 类型 | 说明 | 示例 |
|------|------|------|
| M | 目录（顶级菜单分组） | 系统管理、系统监控 |
| C | 菜单（可访问的页面） | 用户管理、角色管理 |
| F | 按钮（页面内操作权限） | 用户查询、用户新增 |
...
- **双 Token 机制**：访问令牌（2 小时）+ 刷新令牌（7 天）
- 登录返回两个 Token，前端分别存储
- 请求拦截器自动携带访问令牌（`Authorization: Bearer xxx`）
- 访问令牌过期时，前端自动用刷新令牌换取新访问令牌，用户无感知
- 登录接口限流：同一 IP 60 秒内最多 5 次请求
- 自动记录登录日志（IP、浏览器、操作系统、成功/失败）

### 2. 权限管理

- **用户管理**：增删改查用户，分配角色，重置密码，启用/禁用
- **角色管理**：增删改查角色，分配菜单权限
...
- 查看和编辑个人信息（昵称、邮箱、手机号）
- 修改密码（需验证旧密码）
- 头像上传（支持 JPG/PNG/GIF/WebP，最大 2MB）
- 入口：右上角用户名下拉菜单 → 个人中心
...
- 预置字典：用户性别、系统状态、菜单类型、操作类型等
- 接口：`GET /dict/data/type/{dictType}`

### 7. 文件上传与管理

- 支持单文件和批量上传，拖拽上传，图片缩略图预览
- 按日期自动分目录存储（`uploads/2026/05/28/xxx.jpg`）
- UUID 重命名，避免文件名冲突
- 支持图片、文档、压缩包、音视频等多种格式
- 文件大小和类型校验，上传失败自动回滚（磁盘文件 + 数据库记录）
- 图片在线预览，其他文件下载
- **文件管理列表**：查看所有已上传文件，支持删除、复制链接、缩略图预览
- 所有配置通过 `application.yml` 管理，可通过环境变量覆盖

### 8. 通知公告

- 两种类型：**通知**（个人、时效性）和 **公告**（全局、信息性）
- 支持草稿和已发布两种状态
- **SSE 实时推送**：管理员发布公告/通知后，所有在线用户立即收到弹窗提醒
- **通知铃铛**：布局顶部铃铛图标，显示未读通知数量，点击查看列表
- **首页公告**：Dashboard 展示最新已发布公告，点击查看详情
- **登录弹窗**：登录后自动弹出最新公告，支持"今日不再提示"
- 已读状态通过 sessionStorage 记录，查看后角标自动消失
- SSE 断线自动重连（5 秒间隔）
...
| GET | `/admin/auth/profile` | 获取个人信息 | 是 |
| PUT | `/admin/auth/profile` | 修改个人信息 | 是 |
| PUT | `/admin/auth/password` | 修改密码 | 是 |
| POST | `/admin/auth/refresh` | 刷新访问令牌 | 否（需 refreshToken） |
...
| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/admin/file/upload` | 上传单个文件 | 是 |
| POST | `/admin/file/upload/batch` | 批量上传 | 是 |
| GET | `/admin/file/list` | 文件列表 | 是 |
| DELETE | `/admin/file/{id}` | 删除文件 | 是 |
| GET | `/admin/file/view/**` | 文件预览/下载 | 否 |

### 通知公告

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/admin/notice/list` | 公告列表（管理端，含草稿） |
| GET | `/admin/notice/published` | 已发布公告列表 |
| GET | `/admin/notice/{id}` | 公告详情 |
| POST | `/admin/notice` | 新增公告（发布时 SSE 推送） |
| PUT | `/admin/notice` | 修改公告 |
...
| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/admin/sse/subscribe?token=xxx` | 订阅通知推送 | URL 参数传 Token |
...
jwt:
  ...
  expiration: 7200000            # 访问令牌有效期（2小时）
  refresh-expiration: 604800000  # 刷新令牌有效期（7天）

upload:
  path: ${user.dir}/uploads
  allowed-extensions: jpg,jpeg,png,gif,bmp,webp,ico,svg,pdf,doc,docx,xls,xlsx,ppt,pptx,txt,zip,rar,7z,mp3,mp4,avi,mov
  max-size: 10
```
...
VITE_APP_API_TARGET = http://localhost:8080
```
...
以下功能已规划但尚未实现，可根据需要逐步开发：

| 优先级 | 功能 | 说明 |
|--------|------|------|
| 中 | 部门管理 | 组织架构树，用户按部门分组 |
| 中 | 数据导入导出 | 用户/角色等数据的 Excel 导入导出 |
| 低 | 系统监控 | 服务器信息、JVM、Redis 状态面板 |
| 低 | 定时任务 | 基于 Quartz 的任务调度管理 |
| 低 | 代码生成器 | 根据数据库表自动生成 CRUD 代码 |
| 低 | 国际化 | 中英文切换 |
